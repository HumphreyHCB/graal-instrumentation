...
# JVMCI_VERSION_CHECK=ignore JDK_VERSION_CHECK=ignore JAVA_HOME=/usr/lib/jvm/java-11-openjdk-11.0.25.0.9-2.el9.x86_64 mx c1visualizer
 ./latest_graalvm_home/bin/java \
  -Djdk.graal.EnableGTSlowDown=false -Djdk.graal.LIRGTSlowDown=false -Djdk.graal.GTMarkBasicBlocks=true -Djdk.graal.LIRBlockSlowdownFileName=BlockSlowdown1.json \
 -XX:+UnlockExperimentalVMOptions -XX:+UnlockDiagnosticVMOptions -XX:+EnableJVMCI -Djdk.graal.Dump=CodeGen -Djdk.graal.PrintBackendCFG=false  -Djdk.graal.LogFile=AssemblyOut.txt \
  -XX:+UseJVMCICompiler -XX:-UseJVMCINativeLibrary -XX:-TieredCompilation -XX:-BackgroundCompilation '-XX:CompileCommand=dontinline,*::*' -Djdk.graal.TrivialInliningSize=0 \
