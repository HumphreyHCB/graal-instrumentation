...
/**
 * Off-heap, cross-isolate loop-call-count cache.
 *
 * Each entry is:
 *   "<compId>:<loopId> <value>"
 ...
 */
public final class BuboNativeLoopLoopCallCountCache {
    ...
    /**
     * Store integer value for (compId, loopId).
     * Will overwrite older entries in ring fashion.
     */
    public static void add(int compId, int loopId, int value) {
        ensureInitialized();
        ...
        // format: "<compId>:<loopId> <value>"
        String payload = compId + ":" + loopId + " " + value;
     ...
     * Last writer wins.
     */
    public static Map<Integer, Map<Integer, Integer>> snapshot() {
        ensureInitialized();
        Unsafe u = unsafe();
        u.loadFence();
        ...
        for (long i = start; i < written; i++) {
            ...
            // s = "<compId>:<loopId> <value>"
            int spacePos = s.indexOf(' ');
            if (spacePos <= 0) {
                continue;
            }
            String key      = s.substring(0, spacePos);
            String valueStr = s.substring(spacePos + 1);
